...
class PanoptesRedisSentinelConnectionConfiguration(object):
    ...
    def __init__(self, sentinels, master_name, db, master_password=None):
        ...
        for sentinel in sentinels:
            ...
            self._sentinels.append(Sentinel(host=url.group("host"), port=port, password=password))
    ...
    def __repr__(self):
        ...
        return ",".join(
            [
                "sentinel://{}{}{}".format(":**@" if sentinel.password else "", sentinel.host, ":" + str(sentinel.port))
...
class PanoptesConfig(object):
    ...
    def __init__(self, logger, conf_file=None):
        ...
        if kafka_config["publish_to_site_topic"] is False and kafka_config["publish_to_global_topic"] is False:
        ...
        if not (kafka_config["publish_to_site_topic"] and kafka_config["consume_from_site_topic"]):
            if (kafka_config["publish_to_site_topic"] and not kafka_config["consume_from_site_topic"]) or (
                ...
                raise PanoptesConfigurationError(
                    "Panoptes metrics will not be consumed. The consumer is set to "
                    "consume from the incorrect topic. Change either `publish_to_"
                    "site_topic` or `publish_to_global_topic` in the site wide "
                    "configuration file"
                )
        ...
        for plugin_type in const.PLUGIN_TYPES:
            ...
            for plugins_path in plugins_paths:
                if not os.path.isdir(plugins_path):
                    raise Exception('%s plugins path "%s" does not exist or is not accessible' % (plugin_type, plugins_path))
    ...
    def _setup_logging(self, config):
        ...
        self._logger.info(f"Logging configuration file: {Path(log_config_file).resolve()}")
        ...
        throttle = RateLimitingFilter(rate=config["log"]["rate"], per=config["log"]["per"], burst=config["log"]["burst"])
    ...
    def _get_redis_urls(self, config):
        ...
        for group_name in config["redis"]:
            ...
            if namespace in redis_urls_by_namespace:
                raise ValueError('Invalid Redis configuration: namespace "{}" is configured multiple times'.format(namespace))
                ...
                if "host" in shard and shard["host"] is not None:
                    if "sentinels" in shard and shard["sentinels"] is not None:
                        raise ValueError(
                            ...
                            'shard "{}" in group "{}" has both "host" and "sentinels" configured'.format(shard_name, group_name)
                        )
                    else:
                        connection = PanoptesRedisConnectionConfiguration(
                            host=shard["host"], port=shard["port"], db=shard["db"], password=shard["password"]
                        )
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Invalid Redis configuration: "
                            'shard "{}" in group "{}" has invalid sentinel configuration'.format(shard_name, group_name)
                        )
                else:
                    raise ValueError(
                        "Invalid Redis configuration: "
                        'shard "{}" in group "{}" has neither "host" or "sentinels" configured'.format(shard_name, group_name)
                    )
        ...
        if const.DEFAULT_REDIS_GROUP_NAME not in redis_urls_by_group:
            raise ValueError(
                'Invalid Redis configuration: no "{}" group found. Configuration has the following '
                "groups: {}".format(const.DEFAULT_REDIS_GROUP_NAME, list(redis_urls_by_group.keys()))
            )
    ...
    def _get_zookeeper_servers(self, config):
        ...
        for zookeeper_server_id in config["zookeeper"]["servers"]:
            zookeeper_server = (
                config["zookeeper"]["servers"][zookeeper_server_id]["host"]
                + ":"
    ...
    def _get_kafka_brokers(self, config):
        ...
        for kafka_broker_id in config["kafka"]["brokers"]:
            kafka_broker = (
                config["kafka"]["brokers"][kafka_broker_id]["host"]
                + ":"
